...
function MotorService(dataProvider,logger, callback) {
	...
	TraerDatos(dataProvider, function(error, data) {
		console.log("Finaliza Inicializacion de datos de MotorService");
	});
}
	 ...
	 dataProvider.Device().GetAll(function (err, data){
  			if (err) {
	   		...
	   		if (data && data.length > 0)
	   		{
		          ...
		          }, function(error) { if (_DEBUG) console.log("Error al leer async en mettodo Device.GetAll() error : " + error); });
		     }
  ...
  dataProvider.Motor().GetAll(function (err, data){
  			if (err) {
	   		...
	   		if (data && data.length > 0)
	   		{
		          ...
		          }, function(error) { if (_DEBUG) console.log("Error al leer async la coleccion Motor.GetAll() error : " + error); });
		     }
...
function MotorExists(lst, value) {
	...
	}, function(error) { if (_DEBUG) console.log("Error al leer async en metodo MotorExists error : " + error); retorno = false; });
