...
method.GetTemperatura = function(idDevice, idSensor, callback) {
 	if (_DEBUG)
 		console.log("SensorService.GetTemperatura() - Llamando servicio");
	...
	var params;
	 async.each(dispositivos, function(doc, cb) {
	  	if (doc.Id == idDevice) {
			  	...
			  	if (result.IdDispositivo == idDevice && result.IdSensor == idSensor)
			  	{
			  		if (result.Habilitado) {
			  			...
			  			if (funcion) {
				  			...
				  			{
				  				case "Temperatura":
				  					params = "T" + idSensor+result.Pin;
				  					break;
				  					
				  				case "Humedad":
				  					params = "H" + idSensor+result.Pin;
				  					break;
			  			...
			  			console.log("Sensor.LlamarServicio - Params: " + params);
			  				...
			  				doc.Servicio.callFunction("Sensor", params, function(error, data) {
					  			
					  				//TODO: Mejorar esto
			  					if (error) {
			  						console.log("Sensor.LlamarServicio - ERROR: " + error);
			  ...
			  }, function(error) { if (error) console.log("SensorService.LlamarServicio.LecturaAsyncSensores : error : " + error) });
	  		
	  		
	  	}
	  }, function(error) { if (error) console.log("SensorService.LlamarServicio.LecturaAsyncDispositivos : error : " + error) });
}
