	 ...
	 if (_provider.NEDB){
		 db.Temporizador.find({}, function (err, docs) {
			...
			if (docs.length == 0) {
			     ...
			     console.log("Inicializo base de datos con valores desde archivo de datos iniciales /db/DatosIniciales/Temporizador.json");
...
method.Delete = function(IdTemporizador) {
	 if (_provider.NEDB){
	 	db.Temporizador.remove({ IdTemporizador: IdTemporizador }, {}, function (err, numRemoved) {
  			if(_DEBUG)
  				console.log("Temporizador.Delete -> CantEliminados " + numRemoved); 
		});
	 }
};
...
method.Save = function(IdTemporizador, IdDispositivo, IdTipoActuador, IdActuador, Descripcion,DuracionMinutos,NumeroDias, HorasActivacion,Habilitado ) {
    ...
    if (_provider.MONGO) {
        throw new Error("Temporizador.Save no habilitado para MongoDB");
    }
...
method.GetCollection = function(filter, callback) {
 ...
 if (_provider.MONGO) {
     ...
     throw new Error("Temporizador.GetCollection() no habilitado para Mongo");
  }
  ...
  if (_provider.NEDB) {
      ...
      if (filter.IdActuador)
       ...
       if (filters.length == 0) {
	      	if (_DEBUG)
	      		console.log("Temporizador.method.Find() no se han seteado filtros"); 
	      }
          ...
          async.each(docs, function(doc, cb) { 
            var TemporizadorModel = new Models();
              ...
              TemporizadorModel.Crear(doc.IdTemporizador,
			        				  ...
			        				  doc.Habilitado);
			if (_DEBUG)			        				  
            	console.log("Temporizador.GetCollection() : " + TemporizadorModel.Objeto());
...
method.GetAll = function(callback) {
 ...
 if (_provider.MONGO) {
     ...
     throw new Error("Temporizador.GetAll() no habilitado para Mongo");
  }
          ...
          async.each(docs, function(doc, cb) { 
            var TemporizadorModel = new Models();
              ...
              TemporizadorModel.Crear(doc.IdTemporizador,
			        				  ...
			        				  doc.Habilitado);
			if (_DEBUG)			        				  
            	console.log("Temporizador.GetAll() : " + TemporizadorModel.Objeto());
            lstModels.push(TemporizadorModel.Objeto());
...
method.GetLast = function(filter, callback) {
  ...
  if (_provider.MONGO) {
    ...
    console.log("TemporizadorProvider.GetLast no implementado para Mongo");
  }
  ...
  if (_provider.NEDB) {
    ...
    console.log("TemporizadorProvider.GetLast no implementado para NEDB");
  }
...
method.Find = function(filter, callback) {
	...
	if (_provider.NEDB) {
	      ...
	      if (filters.length == 0) {
	      	if (_DEBUG)
	      		console.log("method.Find() no se han seteado filtros"); 
	      }
