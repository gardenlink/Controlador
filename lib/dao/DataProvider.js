 ...
 function EstablecerConfiguraciones(config) {
    for (var conf in config.datasource) {
      ...
      if (conf == "NEDB")
      {
      	if (Configuracion[conf].Habilitado == "true")
      	{
      		var dbPath = path.join(path.resolve(__dirname,'..', '..'),"db", path.sep);
      		Configuracion[conf].Path = dbPath;
      		console.log("DataProvider.EstablecerConfiguraciones -> Base Path para BD : " + dbPath);
      	}
      }
    }
 }
...
method.Cache = function(enabled,callback) {
	if (enabled && datosConsolidados.length > 0) {
		...
		return callback(null, datosConsolidados);	
	}
	else
	{
		TraerDatos(callback);
