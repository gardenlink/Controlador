...
function SensorProvider(logs, moment, config) {
  ...
  _logs = logs;
    ...
    if (config.NEDB.Habilitado == "true"){
      ...
      db.Sensor = new Datastore({ filename: dbpath + 'Sensor.db', autoload:true});
...
method.Save = function(IdSensor, IdDispositivo, Descripcion, MarcaModelo, Tipo,Pin,EsPinAnalogo,Habilitado,callback) {
    ...
    if (_provider.MONGO) {
      ...
      sensorModel.Crear(IdSensor, 
      					...
      					Habilitado);
      if (_DEBUG)
        console.log("<MONGO> SensorProvider.Save - Objeto que se va a guardar : " + JSON.stringify(sensorModel.Objeto()));
     ...
     if (_provider.NEDB){
      ...
      db.Sensor.find({IdSensor : sensorModel.Objeto().IdSensor}, function (err, data) {
        ...
        {
          //al usar find, devuelve un arreglo. FindOne devuelve un objeto
