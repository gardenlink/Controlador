...
function DeviceProvider(logs, config, debug) {
  ...
  _logs = logs;
    ...
    if (config.NEDB.Habilitado == "true"){
      ...
      db.Device = new Datastore({ filename: dbpath + 'Dispositivo.db', autoload:true});
...
method.Save = function(Id, Nombre, Tipo, Ip,Puerto,Habilitado,Estado,FrecuenciaMuestreo, callback) {
        ...
        deviceModel.Crear(Id, Nombre, Tipo, Ip, Puerto, Habilitado,Estado,FrecuenciaMuestreo);
         ...
         if(deviceModel.Validar(deviceModel.Objeto())) {
              ...
              else
              {
                ...
                if (callback) 
					return callback("<MONGO> DeviceProvider.Save() : No se graba objeto debido a que no pasa la validacion de integridad",null);
              }
    ...
    if (_provider.NEDB){
      ...
      else
      {
        ...
        if(callback)
			return callback("<NEDB> DeviceProvider.Save() : No se graba objeto debido a que no pasa la validacion de integridad",null);
      }
    }
...
method.Delete = function(IdDevice, callback) {
	 if (_provider.NEDB){
         ...
         if (docs.length > 0) {
          async.each(docs, function(doc, cb) { 
            ...
            lstModels.push(deviceModel.Objeto());
          	...
          	if (_DEBUG) 
          		console.log("<MONGO> DebiceProvider.Find(): Error al leer async error : " + error);
...
method.GetCollection = function(filter, callback) {
 ...
 if (_provider.MONGO) {
  ...
  if (_provider.NEDB) {
      ...
      db.Device.find(filters, function (err, docs) {
        
        if (err) {
           if (_DEBUG) console.log("<NEDB> DeviceProvider.GetCollection : Error al leer async en mettodo GetAll() error : " + error);
           return callback(err, null);
        } 
        else 
        {
          ...
          async.each(docs, function(doc, cb) { 
            ...
            deviceModel.Crear(doc.Id, 
           	...
           	if (_DEBUG) console.dir("<NEDB> DeviceProvider.GetCollection : Objeto Encontrado : " + deviceModel.Objeto());
          ...
          {
          	  if (_DEBUG) console.log("<NEDB> DeviceProvider.GetCollection: Registro no encontrado");
	          if (callback)
	          	return callback(null, lstModels);
          }
        }
