...
function DeviceProvider(logs, config) {
  ...
  _logs = logs;
    ...
    if (config.datasource.DWEET.Habilitado == "true"){
      ...
      _dweet_prefijo = config.datasource.DWEET.prefijo_dispositivo;
    }
...
method.Find = function(filter, callback) {
 ...
 if (_provider.MONGO) {
   ...
   throw new Error("DeviceProvider.Find.Mongo - No implementado");
  }
...
if (docs.length == 0) {
     ...
     console.log("Inicializo base de datos con valores desde archivo de configuracion");
     /*
     var Device1 = {
