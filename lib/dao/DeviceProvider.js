...
method.Delete = function(IdDevice) {
	 if (_provider.NEDB){
	 	db.Device.remove({ Id: IdDevice }, {}, function (err, numRemoved) {
  			if(_DEBUG)
  				console.log("Sensor.Device -> CantEliminados " + numRemoved); 
		});
	 }
};
    ...
    db.Device.find({Id : filter.Id}, function (err, docs) {
      ...
      if (docs.length > 0) {
        async.each(docs, function(doc, cb) { 
          ...
          lstModels.push(deviceModel.Objeto());
          if (_DEBUG)
        	console.log("Find : " + deviceModel.Objeto().Id);
        }, function(error) { 
        	console.log("Error al leer async en mettodo GetAll() error : " + error);
        	callback(error, null); 
        });
      }
      callback(null, lstModels[0]);
