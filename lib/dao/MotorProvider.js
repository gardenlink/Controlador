...
function MotorProvider(logs, moment, config) {
  ...
  _logs = logs;
    ...
    if (config.NEDB.Habilitado == "true"){
      ...
      dbpath = (process.env.HOME || process.env.HOMEPATH || process.env. USERPROFILE) + '/GardenLink/Controlador/db/';
      db.Motor = new Datastore({ filename: dbpath + 'Motor.db', autoload:true});
...
method.Delete = function(IdMotor, callback) {
	 if (_provider.NEDB){
...
method.Save = function(IdMotor, IdDispositivo, Descripcion, MarcaModelo, Tipo,Pin,EsPinAnalogo, Habilitado,Posicion,Accion,Estado, callback) {
        ...
        var MotorModel = new Models();
         ...
         if(MotorModel.Validar(MotorModel.Objeto())) {
              ...
              else
              {
                ...
                if (callback)
                	return callback("<MONGO> MotorProvider.Save() : No se graba objeto debido a que no pasa la validacion de integridad",null)
              }
      ...
      if (_provider.NEDB){
	      ...
	      else
	      {
	        if (_DEBUG)
	         ...
	         if(callback)
				return callback("<NEDB> MotorProvider.Save : No se graba objeto debido a que no pasa la validacion de integridad", null);
	      }
    }
 ...
 if (_provider.MONGO) {
     ...
     var lstModels = [];
      ...
      MongoModels.find(filters, function (err, docs) {
        ...
        if (err) {
           if (_DEBUG) console.log("<MONGO> MotorProvider.GetCollection : Error al leer async error : " + err);
  ...
  if (_provider.NEDB) {
      ...
      db.Motor.find(filters, function (err, docs) {
        ...
        if (err) {
           if (_DEBUG) console.log("<NEDB> MotorProvider.GetCollection : Error al leer async  error : " + err);
           return callback(err, lstModels);
        } 
        else 
        {
...
method.Find = function(filter, callback) {
	...
	if (_provider.MONGO) {
	    ...
	    MongoModels.find(filter).exec(function(err, docs) {
	      if (err) {
	           if (_DEBUG) console.log("<MONGO> MotorProvider.Find() : Error al leer async en mettodo Find() error : " + err);
	           return callback(err, null);
	        } 
	        else 
	        {
	...
	if (_provider.NEDB) {
	      ...
	      db.Motor.find(filters, function (err, docs) {
	        ...
	        if (err) {
	           if (_DEBUG) console.log("<NEDB MotorProvider.Find() : Error al leer async en mettodo Find() error : " + err);
	           return callback(err, null);
	        } 
	        else 
	        {
	          if (docs.length > 0) {
		          ...
		          if (_DEBUG) console.log("<NEDB> MotorProvider.Find() : Error al leer async  error : " + error); 
		          	return callback(error, null);
		          });
	          }
	          return callback(null, lstModels[0]);
 ...
 if (_provider.MONGO) {
      ...
      var lstModels = [];
       MongoModels.find({}).exec(function(err, docs) {
        ...
        if (err) {
           if (_DEBUG) console.log("<MONGO> MotorProvider.GetAll() : Error al leer async en mettodo GetAll() error : " + err);
           return callback(err, null);
        } 
        else 
        {
