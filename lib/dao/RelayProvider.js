  ...
  _logs = logs;
    ...
    if (config.MONGO.Habilitado == "true") {
      ...
      var RelaySchema = new Schema({ 
      	IdRelay: { type: Number, index: { unique: true }, required : true }
    ...
    if (config.NEDB.Habilitado == "true"){
      ...
      db.Relay = new Datastore({ filename: dbpath + 'Relay.db', autoload:true});
...
method.Delete = function(IdRelay, callback) {
	 if (_provider.MONGO) {
...
method.Save = function(IdRelay, IdDispositivo, Descripcion, MarcaModelo, Tipo,Pin,EsPinAnalogo, Habilitado, Activo,EsInverso,callback ) {
        ...
        var relayModel = new Models();
	      ...
	      else
	      {
	        if (_DEBUG)
	          ...
	          if (callback)
	          	return callback("<MONGO> RelayProvider.Save() : No se graba objeto debido a que no pasa la validacion de integridad", null);
	      }
    }
      ...
      if (_provider.NEDB){
	      ...
	      else
	      {
	        if (_DEBUG)
	         ...
	         if(callback)
                  	return callback("<NEDB> RelayProvider.Save() : No se graba objeto debido a que no pasa la validacion de integridad", null);
	      }
    }
...
method.GetLast = function(filter, callback) {
  ...
  if (_provider.MONGO) {
    ...
    MongoModels.findOne().sort(sortObject).exec( function(err, doc) {
	     if (err)
	     	return callback(err, null);
	     else {
  ...
  if (_provider.NEDB) {
     ...
     db.Relay.findOne().sort("IdRelay", 1).exec( function(err, doc) {
	     if (err)
	     	return callback(err, null);
	     else {
	     	return callback(null, doc);
 ...
 if (_provider.MONGO) {
     ...
     var lstModels = [];
      MongoModels.find({}).exec(function(err, docs) {
        ...
        if (err) {
           if (_DEBUG) console.log("<MONGO> RelayProvider.GetAll: Error al leer async en mettodo GetAll() error : " + err);
           return callback(err, lstModels);
        } 
        else 
        {
