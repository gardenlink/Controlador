...
IPAddress server(192,168,200,84); //Master Server
char myIPAddress[20];

RestClient restClient = RestClient("192.168.200.84",9000); //Master server
...
long updateFrequency = 10000UL;    // Update frequency in milliseconds (20000 = 20 seconds). Change this to change your sample frequency.
...
void actualizarSensores(String idSensor, int valor) {
    ...
    if (client.connect(server,PUERTO_REST_API))
    {
      ...
      client.println("Host: 192.168.200.84");
...
void subscribirDispositivo(String ip) {
    ...
    //Llamada a servicio rest para subscribir dispositivo y obtener payload
    int statusCode = restClient.get("/Dispositivo/ConsultarEstado/001", &respuesta);
...
/*
   ...
   if (client.connect(server,PUERTO_REST_API))
   {
      ...
      client.println("PUT /Dispositivo/SubscribirDispositivo/" + (String)ID_DISPOISTIVO + " HTTP/1.1");
      client.println("Host: 192.168.200.84");
      client.println("User-Agent: Arduino/1.0");
      client.println("Content-Type:  application/x-www-form-urlencoded");
      client.println("Connection: close");
